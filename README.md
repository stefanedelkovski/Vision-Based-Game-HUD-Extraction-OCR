...
├── evaluation_images/    # Sample images used for testing performance or training evaluation.
├── inference_flask_scripts/    # Supporting scripts used by the Flask inference server for handling requests, preprocessing, and postprocessing.
├── model_inferences/    # Stored inference outputs, logs or prediction results generated by the models.
├── models/    # Pretrained model weights and configuration files used for detection and OCR.
├── node-scraper-v3.1/    # Node.js-based scraper used for collecting frames (e.g., extracting frames from video sources).
├── yolov5/    # YOLOv5 detection framework used for identifying HUD regions and visual elements.
├── app.py    # Main Flask application entry point. Runs the inference API and exposes endpoints for HUD extraction.
├── count_files.py    # Utility script for counting files within directories (used for dataset or output inspection).
├── env.yaml    # Defines the Conda environment including Python version and dependencies.
├── game_detection_efficientnet_inference.py    # Runs inference using an EfficientNet-based vision model to detect relevant HUD areas. 
├── model_inferences_and_output.py    # Combines raw model predictions with postprocessing logic and structured output formatting.
├── requirements_og.txt
├── stream_worker4.py    # Processes video streams or batches of frames continuously, applying the detection + OCR pipeline.
└── README.md
